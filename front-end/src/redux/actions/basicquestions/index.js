...
export const getData = params => {
  console.log("Get Data Parmas: ", params);

    return async dispatch => {
      await Axios.get("settings/get_basic_data", params).then(response => {
        if(!response.data.status){
          ...
          dispatch({ type: "GET_BASIC_ALL_DATA", data: rdata.data })
          dispatch({
            type: "GET_BASIC_DATA",
            data: fdata,
  ...
  export const getInitialData = () => {
    return async dispatch => {
      await Axios.get("/api/datalist/initial-data").then(response => {
        dispatch({ type: "GET_BASIC_ALL_DATA", data: response.data })
      })
    }
  }
  
  export const filterData = value => {
    return dispatch => dispatch({ type: "FILTER_BASIC_DATA", value })
  }
  
  export const deleteData = obj => {
    return dispatch => {
      Axios
        .post("settings/basic_delete",obj)
        .then(response => {
          if(!response.data.status){
  ...
  export const updateData = obj => {
    ...
    return (dispatch, getState) => {
      Axios
        .post("settings/basic_update", obj)
        .then(response => {
          if(!response.data.status){
            toast.error(response.data.msg);
          }
