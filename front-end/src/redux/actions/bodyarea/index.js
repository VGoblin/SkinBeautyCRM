...
export const getData = params => {
  console.log("Get Data Parmas: ", params);
    ...
    return async dispatch => {
      await Axios.get("settings/getdata", params).then(response => {
        if(!response.data.status){
          ...
          dispatch({ type: "GET_AREA_ALL_DATA", data: rdata.data })
          dispatch({
            type: "GET_AREA_DATA",
            data: fdata,
  ...
  export const getInitialData = () => {
    return async dispatch => {
      await Axios.get("/api/datalist/initial-data").then(response => {
        dispatch({ type: "GET_AREA_ALL_DATA", data: response.data })
      })
    }
  }
  
  export const filterData = value => {
    return dispatch => dispatch({ type: "FILTER_AREA_DATA", value })
  }
  
  export const deleteData = obj => {
    return dispatch => {
      Axios
        .post("settings/delete",obj)
        .then(response => {
          if(!response.data.status){
